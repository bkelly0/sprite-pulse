...
export type SpriteSheetBundleSource =
  SpriteSheetBundle | SpritePulseAtlasAssetSource | TexturePackerAssetSource;
...
export class SpriteSheetBundle {
  ...
  public static fromSource(source: SpriteSheetBundleSource): SpriteSheetBundle {
    ...
    if ("atlasMetadata" in source) {
      return SpriteSheetBundle.fromAtlasMetadata(
        source.id,
  ...
  public createSingleFrameSpriteSheets() {
    for (const imageFile of this.imageFiles) {
      const spriteSheet = this.createSingleFrameSpriteSheet(imageFile);
