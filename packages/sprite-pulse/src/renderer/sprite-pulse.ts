...
export class SpritePulse {
  ...
  private loopFrameRequestId: number | null = null;
  private loopRunning = false;
  private loopStartTimestamp = 0;
  private loopLastFrameTimestamp = 0;
  private loopFrameCount = 0;
  ...
  public startLoop(
    callback: SpritePulseLoopCallback,
    options: SpritePulseLoopOptions = {},
  ): void {
    if (this.isDisposed) {
      return;
    }
    ...
    const targetFps = Math.max(1, options.targetFps ?? 60);
    const frameIntervalMs = 1000 / targetFps;
    const shouldThrottle = targetFps < 60;
    ...
    const runFrame = (timestamp: number): void => {
      ...
      if (this.loopStartTimestamp === 0) {
        this.loopStartTimestamp = timestamp;
        this.loopLastFrameTimestamp = timestamp;
      }

      const deltaMs = timestamp - this.loopLastFrameTimestamp;
      this.loopLastFrameTimestamp = timestamp;

      if (shouldThrottle) {
        this.loopAccumulatorMs += deltaMs;
        if (this.loopAccumulatorMs < frameIntervalMs) {
          this.loopFrameRequestId = requestAnimationFrame(runFrame);
          return;
        }
      }

      const effectiveDeltaMs = shouldThrottle
        ? this.loopAccumulatorMs
        : deltaMs;
      this.loopAccumulatorMs = 0;
      this.loopFrameCount += 1;

      const frame: SpritePulseLoopFrame = {
        timestamp,
        deltaMs: effectiveDeltaMs,
        elapsedMs: timestamp - this.loopStartTimestamp,
        frame: this.loopFrameCount,
      };
      ...
      try {
        ...
        this.stopLoop();
        this.reportLoopError(error, options.onError);
        return;
      }
  ...
  public stopLoop(): void {
    ...
    if (this.loopFrameRequestId !== null) {
      cancelAnimationFrame(this.loopFrameRequestId);
      this.loopFrameRequestId = null;
    }

    this.loopStartTimestamp = 0;
    this.loopLastFrameTimestamp = 0;
    ...
    for (const layer of layers) {
      for (const sprite of layer.sprites) {
        ...
        const viewportY = Math.round(
          targetHeight - renderRect.y - renderRect.height,
        );
  ...
  private reportLoopError(
    ...
    const exception =
      error instanceof Error ? error : new Error(`Loop error: ${String(error)}`);
  ...
  private createBundleAtlas(
    ...
    for (const { imageFile, image } of loadedImages) {
      ...
      frames.set(
        imageFile,
        new Rect(cursorX, cursorY, image.width, image.height),
      );
    ...
    if (!context) {
      throw new Error(
        "Failed to create a 2D canvas context for bundle atlas generation.",
      );
    }
